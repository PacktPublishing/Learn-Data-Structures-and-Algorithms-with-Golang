...
func main() {
	...
	numNodes = 2 * 45
	var nodes []GraphNode
	nodes = make([]GraphNode, numNodes+1)
	var nodeMap map[int](*GraphNode)
	nodeMap = make(map[int](*GraphNode))

	var nodeBytes []byte
	var err error
	nodeBytes, err = ioutil.ReadFile("graph_nodes.json")
	...
	for i, node = range nodes {
		nodeMap[node.Identifier] = &nodes[i]
	}

	var edges DeliveryPersonRouteGraph

	var edgeBytes []byte
	edgeBytes, err = ioutil.ReadFile("graph_edges.json")

	if err != nil {
		panic(err)
	}
	err = json.Unmarshal(edgeBytes, &edges)
	if err != nil {
		panic(err)
	}
	...
	for i, edge = range edges {
		var found bool
		edges[i].Current, found = nodeMap[(*edge.Current).Identifier]

		if !found {
			panic("unable to create a map ")
		}
		edges[i].Neighbor, found = nodeMap[(*edge.Neighbor).Identifier]
		if !found {
			panic("unable to create a map")
		}
	}
	...
	var graph DeliveryPersonRouteGraph
	graph = GetOptimalPlan(nodes, edges)


	if evaluateDeliveryPersonRouteGraph(graph, nodes) {
		printDeliveryPersonRouteGraph(graph)
		os.Exit(0)
	}

	for i = 0; i < len(edges); i++ {
		var edgeset DeliveryPersonRouteGraph
		edgeset = append(edges[:i], edges[i+1:]...)
		graph = GetOptimalPlan(nodes, edgeset)
		if evaluateDeliveryPersonRouteGraph(graph, nodes) {
			printDeliveryPersonRouteGraph(graph)
			os.Exit(0)
		}
