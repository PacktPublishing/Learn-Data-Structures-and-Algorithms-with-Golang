...
// SortSRT method
func SortSRT(processes [3]OSProcess) []OSProcessStep {
...
func getSortedProcesses(processes [3]OSProcess, timePeriod int) []OSProcess {
  ...
  sort.Slice(cProcesses, func(i int, j int) bool {
    if cProcesses[i].GetLeftBurstTime() < cProcesses[j].GetLeftBurstTime() {
      return true
    } else if cProcesses[i].GetLeftBurstTime() < cProcesses[j].GetLeftBurstTime() {
